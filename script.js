...
let totalPoints = 0; // Общий баланс пользователя
...
function createStar() {
  ...
  // Удаляем звезду через 4 секунды
  setTimeout(() => {
    star.style.opacity = 0;
    setTimeout(() => {
      star.remove();
...
// Постоянное добавление звезд
setInterval(createStar, 300); // Новая звезда каждые 300 мс